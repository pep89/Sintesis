				...
				{	
					...
					$result = mysql_query($anysnocreats);
						...
						if(mysql_num_rows($result) < 1)
						{
							...
							{
								$idany = $row["id"];
								//echo "año creado con identificador" . $idany . "</br>";
							}
						...
						else
						{
							...
							{
								$idany = $row["id"];
								//echo "Año existente con identificador" . $idany . "</br>";
				...
				else if ($c == 4)
				{
					...
					$hores = $datos[$c + 2];
						...
						if(mysql_num_rows($resultclases) < 1)
						{
							...
							{
								$idclase = $row["id"];
								//echo "clase creada con identificador" . $idclase . "</br>";
							}
							
						}
						else
						{
							$claseexistent = "Select id from clases";
							$idexistent = mysql_query($claseexistent);
							while($row = mysql_fetch_array($idexistent))
							{
								$idclase = $row["id"];
								//echo "Clase existente con identificador" . $idclase . "</br>";
				...
				else if ($c == 7)
				{
					...
					$horesclase = $datos[$c + 1];
						...
						if(mysql_num_rows($resultgrups) < 1)
						{
							...
							//------------------------------------Seleccion l'id del nou any introduit - el guard dins una variable - i imprimeixc per pantalla el resultat
							$grupcreat = "Select id from grups where nom = '$datos[$c]'";
							$resultid = mysql_query($grupcreat);
							while($row = mysql_fetch_array($resultid))
							{
								$idgrup = $row["id"];
								echo "grupo creado con identificador" . $idgrup . "</br>";
							}
							
						}
						else
						{
							$grupexistent = "Select id from grups";
							$idexistent = mysql_query($grupexistent);
							while($row = mysql_fetch_array($idexistent))
							{
								$idgrup = $row["id"];
								echo "Grupo existente con identificador" . $idgrup . "</br>";
